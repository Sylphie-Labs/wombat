...
connection and runs the FIVE packaged ``ensure_schema(conn)`` functions this product ships —
``wombat.queue``, ``wombat.domain.daily_ledger``, ``wombat.gate.pending_journal_pg``,
``wombat.behavior.event_log``, ``wombat.trail.schema`` — each a ``CREATE TABLE/INDEX IF NOT
EXISTS`` (NG-3: no migration framework, no version table), so calling this on an already-current
database is a safe no-op. ``bootstrap.assemble_runtime`` calls this as the FIRST pg act on the
``replay_pending=True`` posture, before the TK-166 eager replay (Q-104 ruling).
"""
...
def ensure_all_schemas(dsn: str) -> None:
    ...
    Opens ONE psycopg connection (context-managed — always closed), runs the five packaged
    ...
    already-current database raises nothing and changes nothing (idempotent).
    """
