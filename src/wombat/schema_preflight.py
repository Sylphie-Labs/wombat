...
connection and runs the TWELVE packaged ``ensure_schema(conn)`` functions this product ships —
...
(TK-286), ``wombat.user_facts`` (TK-294), ``wombat.chat_turns`` (TK-295), ``wombat.observations``
(TK-310) — each a ``CREATE TABLE/INDEX IF NOT EXISTS`` (NG-3: no migration framework, no version
...
from .observations import ensure_schema as ensure_observations_schema
...
def ensure_all_schemas(dsn: str) -> None:
    ...
    Opens ONE psycopg connection (context-managed — always closed), runs the twelve packaged
    ...
    with psycopg.connect(dsn) as conn:
        ...
        ensure_observations_schema(conn)
        conn.commit()
