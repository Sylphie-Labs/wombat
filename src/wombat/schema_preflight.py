...
connection and runs the ELEVEN packaged ``ensure_schema(conn)`` functions this product ships —
...
(TK-286), ``wombat.user_facts`` (TK-294), ``wombat.chat_turns`` (TK-295) — each a ``CREATE
TABLE/INDEX IF NOT EXISTS`` (NG-3: no migration framework, no version table), so calling this on
an already-current database is a safe no-op. ``bootstrap.assemble_runtime`` calls this as the
FIRST pg act on the ``replay_pending=True`` posture, before the TK-166 eager replay (Q-104
ruling). TK-286 (DEC-63a): WITHOUT this ninth call the ``wombat_seen_events`` table never exists
in production — ``ensure_all_schemas`` runs on the ``replay_pending=True`` boot path
(``bootstrap.py``), the ONE place every real boot creates it.
"""
...
from .chat_turns import ensure_schema as ensure_chat_turns_schema
...
from .user_facts import ensure_schema as ensure_user_facts_schema


def ensure_all_schemas(dsn: str) -> None:
    """Apply every packaged ``ensure_schema`` migration on ``dsn``, idempotently (CR3-1, Q-104).

    Opens ONE psycopg connection (context-managed — always closed), runs the eleven packaged
