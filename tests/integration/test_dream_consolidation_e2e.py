...
class _PassthroughPersonaStage:
    ...
    ``build_dream_pathway`` now also requires a ``persona`` stage, inserted between ``tune`` and
    ``behavior_log`` — this suite's own AC1-AC3 witnesses are all about ``DreamConsolidationStage``,
    so a trivial always-transitions-onward double merely satisfies the shape without asserting
    anything about it (a real ``DreamPersonaStage`` needs a Postgres-backed ``BehaviorEventLog``
    this module has no DSN for)."""
...
class _PassthroughFactsStage:
    """TK-297 mechanical reshape (flagged per the ticket's own sanction, EP-13):
    ``build_dream_pathway`` now also requires a ``facts`` stage, inserted between ``persona`` and
    ``behavior_log`` — this suite's own AC1-AC3 witnesses are all about ``DreamConsolidationStage``,
    so a trivial always-transitions-onward double merely satisfies the shape without asserting
    anything about it (a real ``DreamFactsStage`` needs a Postgres-backed ``ChatTurnStore``/
...
class _PassthroughObserveStage:
    """TK-314 mechanical reshape (flagged per the ticket's own sanction): ``build_dream_pathway``
    now also requires an ``observe`` stage, inserted between ``derive`` and ``behavior_log`` —
...
class _PassthroughBiometricsStage:
    """TK-346 mechanical reshape (flagged per the ticket's own sanction): ``build_dream_pathway``
    now also requires a ``biometrics`` stage, inserted between ``screenpipe`` and
    ``behavior_log`` — this suite's own AC1-AC3 witnesses are all about
    ``DreamConsolidationStage``, so a trivial always-transitions-onward double merely satisfies
    the shape without asserting anything about it (a real ``DreamBiometricsStage``'s own ACs live
    in ``tests/pathways/test_dream_biometrics_stage.py``)."""
