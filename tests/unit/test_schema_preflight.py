...
test_serve_boot.py``): absent it, tests are skipped LOUDLY. Spin up a throwaway Postgres locally:
  ...
  AC1 a brand-new empty database: ``ensure_all_schemas(dsn)`` creates all nine packaged tables,
...
# The nine packaged tables this pre-flight must create (Q-104-verified module homes; TK-240
# added the sixth, wombat_settings; TK-244 added the seventh, wombat_external_items; TK-247 added
# the eighth, wombat_scratchpad; TK-286 added the ninth, wombat_seen_events).
_PACKAGED_TABLES = (
    "wombat_queue",
    "daily_ledger",
    "pending_journal",
...
@_requires_pg
def test_ac1_ensure_all_schemas_creates_all_nine_packaged_tables(fresh_database: None) -> None:
    assert _DSN is not None
