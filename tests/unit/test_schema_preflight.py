...
test_serve_boot.py``): absent it, tests are skipped LOUDLY. Spin up a throwaway Postgres locally:
  ...
  AC1 a brand-new empty database: ``ensure_all_schemas(dsn)`` creates all ten packaged tables,
...
# The ten packaged tables this pre-flight must create (Q-104-verified module homes; TK-240
...
@_requires_pg
def test_ac1_ensure_all_schemas_creates_all_ten_packaged_tables(fresh_database: None) -> None:
    assert _DSN is not None
