...
def test_stored_token_with_broader_scope_is_rejected_before_credential_use(
    ...
    """TK-168 (CR-3): ``Credentials.from_authorized_user_info(..., scopes=list(GMAIL_SCOPES))``
    overwrites ``creds.scopes`` with the passed constant, so a post-construction check of
    ``creds.scopes`` against ``GMAIL_SCOPES`` checks the constant against itself and can never
    catch a vaulted token that actually granted a broader scope (older consent, manual edit,
    future scope change without re-consent). A stored token carrying ``gmail.modify`` MUST be
    rejected before its credential is ever constructed/refreshed/returned."""
    stored = {
        ...
        "scopes": [*GMAIL_SCOPES, "https://www.googleapis.com/auth/gmail.modify"],
